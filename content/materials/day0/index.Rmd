...
We will use Quarto to render documents. 
Follow the [install instructions on their website](https://quarto.org/docs/get-started/).
...
-   Click on the "Render" button (the blue arrow) to test your installation; everything went well, you should see a new window with an HTML file.

Other important packages are **here**, **renv**, and **devtools**, **roxygen2** and **gitcreds**.
These are on CRAN so you can install them with the usual

``` r
install.packages(c("here", "renv", "devtools", "roxygen2", "gitcreds"))
```
...
The easiest (and most painless) way to do this is to install  [tinytex](ttps://yihui.org/tinytex/) with quarto. 
On a terminal (not your R console), run this command:
...
To get an overview of your quarto installation and what's missing. 
