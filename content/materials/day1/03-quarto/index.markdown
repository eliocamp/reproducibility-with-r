---
title: "03: Reporting with Quarto"
weight: 3
...
An [Quarto](https://quarto.org/) file is a plain text file, with some rules and special syntax that allow you to write code and text together.
...
A new window will open where you can *optionally* complete with the title of your document, author and the output format.
Let’s try HTML output first.
A new file with a template document will open up when you click OK.
This includes a brief demonstration of Quarto capabilities.
You can try to render the file to see the output document.
...
Quarto is a language agnostic implementation of [R Markdown](https://rmarkdown.rstudio.com/).
They share most of their syntax, therefore you can pick up Quarto easily if you’re already familiar with R Markdown.
If you are not, then most of what you learn about Quarto here will be applicable to any R Markdown project that you find in the future.
...
<figure>
<img src="images/quarto.png" alt="Quarto file in the left. Rendered document on the right. The YAML, chunk codes and text areas are highlighted on the file." />
<figcaption aria-hidden="true">Quarto file in the left. Rendered document on the right. The YAML, chunk codes and text areas are highlighted on the file.</figcaption>
</figure>
...
- Below the header goes the document proper, which has *white* and *grey* sections.
  These are the two main sections that make up a Quarto file:
...
The YAML format allows you to define hierarchical lists in a human-readable way.
For example:
...
Many of the errors you’ll find when rendering will be rooted in indentation issues.
...
Plain text sections of a Quarto file is interpreted as Markdown.
You can get a guide to quarto [in this](https://www.markdownguide.org/cheat-sheet/) cheat sheet, but here is a minimum syntax to get you started:
...
The chances of you forgetting to update that “18” are high, that’s why Quarto also has the possibility to incorporate in-line code.
...
<figure>
<img src="images/figure-reference.png" alt="Scatterplot with pressure as a function of temperature labelled “Figure 1” with the previous caption and the text starting with “In Figure 1” with a link to said figure." />
<figcaption aria-hidden="true">Scatterplot with pressure as a function of temperature labelled “Figure 1” with the previous caption and the text starting with “In Figure 1” with a link to said figure.</figcaption>
</figure>
...
If you are here to use Quarto to write papers or reports, you may need to add references to other papers or sources.
...
Besides plain PDFs, Quarto can render to particular styles of PDFs via templates.
For scientific journals you may find the [quarto-journals](https://github.com/quarto-journals/) repository useful, since it includes templates for various popular publishers.
You can find Quarto templates for other journals created by members of the community.
For example, searching for “copernicus quarto template” brings up [this](https://github.com/benfmeyer/quarto-copernicus) repository.
...
<figure>
<img src="images/copernicus_template.png" alt="Sample copernicus template on the left and the rendered version on the right." />
<figcaption aria-hidden="true">Sample copernicus template on the left and the rendered version on the right.</figcaption>
</figure>
...
1.  Create a new Quarto file using one template of your choosing.  
    You can browse [official templates](https://quarto.org/docs/journals/) or search online.
...
Now, what happens if there is no ready-made Quarto template for the journal you want to submit to?
Usually, journals provide LaTeX templates that you can use and **adapt** to use within Quarto.
This will require some knowledge of LaTeX and [Pandoc templates](https://pandoc.org/MANUAL.html#templates), and some patience and coffee/tea/mate to deal with rendering errors.
If you go through the trouble of adapting a template, consider [bundling it to a template](https://quarto.org/docs/journals/formats.html) and releasing it so that others can benefit from your sweat and tears.

<div class="note">

<a href="https://bmcmedicine.biomedcentral.com/articles/10.1186/s12916-023-02882-y#Sec3">One estimate</a> put the amount of work required to format and re-format academic submissions to comply with journal templates at around 230 million USD in 2001.
...
The way rendering to LaTeX work is that we have a LaTeX template with special notation to reference to variables in our document.
So, for example, the template might have a section with
...
It’s easiest to see how this works with a simple, barebones example.
This quarto document has a title, an author, and simple content.
The yaml also specifies that the pdf format will use the file “template.text” as template and will not delete the intermediate LaTeX file after rendering.
...
When rendering, Quarto is going to replace <code> \$body\$</code> with the body of the document and output this LaTeX document:
...
We can use the title variable by using <code> \$title\$</code>.
...
One final element to make templates work are *partials*.
Many Quarto and Pandoc features depend on code that comes with the built-in templates.
These won’t work in our template, which lacks that code.
For example, if we wanted to add code blocks that render in our document like this:
...
This is because Quarto uses the “Shaded” LaTeX environment, that our template doesn’t define, to display code highlighting.
But we don’t need to define it, we can insert the pandoc.tex partial, which includes that (and many other) definitions
...
You can read what partials are available for Quarto in the [LaTeX templates documentation](https://quarto.org/docs/journals/templates.html#latex-partials).

<div class="activity">

1.  Create a new Quarto file using PDF as a output format and save it into your project (for example, `paper.qmd`).

2.  Download the AGU Geophysical Research Letters template from [here](/agujournal2025-latex-template-main.zip).
    Inside the file there’s a folder with a bunch of files. Extract all those files into the same folder you saved the Quarto file.
...
4.  **Render the file**.
    You should get a PDF with the contents of the AGU template and not your document.
    This is to make sure that Quarto is using the template and that the template works.

5.  Add <code> \$pandoc.tex()\$</code> before `\begin{document}` to add support for Quarto and Pandoc features.
...
8.  Now let’s add some customisation.
    Again in the `agujournaltemplate.tex` file, change `\title{=enter title here=}` (line 78) for `\title{$title$}`

9.  **Render the file**.
    Now, the title in the R Markdown file and the PDF should match.
...
10. You can add new options to the YAML header and then use the same trick so they are used by the LaTeX template. For example, add a new option called “abstract” and write some text, like this:
    ...
    Then, in the LaTeX template search for `[ enter your Abstract here ]` and replace it with <code> \$abstract\$</code>.

11. **Render the file**.
    The abstract in the PDF should now match the abstract in your yaml header.

12. (Optional) Add more variables, such as a Plain Language Summary, author, mail of the corresponding author to the yaml heather and modify the LaTeX template to use them.
