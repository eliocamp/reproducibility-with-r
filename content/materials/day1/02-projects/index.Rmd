...
```{r}
# TODO: mover esto para que la ruta de la carpeta no sea tan larga?
fs::dir_tree(here::here("content", "materials", "day1", "02-projects", "bad_project"))
```
...
This is easier said than done, especially due to knitr's default behaviour of setting the location of the .Rmd file as the working directory.
We will see that a simple solution is to use the **here** package.
...
You can also quickly open a recently-used project by clicking on the project icon on the left-hand side of the RStudio toolbar.
...
It will even preserve changes not This helps keep your work tidy and makes it easy to pick up or share what you've done later.
...
::: notes
You can change these preferences at the gobal level with "Tools" -> "Global options" in the "General" subheading
:::
...
However, it can be a bit overwhelming because it has many moving parts, so in this course we will go over each part "by hand" before automatising.
